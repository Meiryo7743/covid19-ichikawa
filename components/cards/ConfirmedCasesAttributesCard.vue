  ...
  <v-col cols="12" md="6" class="DataCard">
    <client-only :placeholder="$t('読み込み中')">
      <data-table
        ...
        :url="'http://www.city.ichikawa.lg.jp/pub01/hasseijokyo.html'"
        :source="$t('オープンデータを入手')"
...
export default {
  ...
  data() {
    ...
    for (const row of patientsTable.datasets) {
      ...
      row['検査確定日'] = this.getTranslatedWording(row['検査確定日'])
      row['発症日'] = this.getTranslatedWording(row['発症日'])
      row['年代'] = this.getTranslatedWording(row['年代'])
      row['性別'] = this.getTranslatedWording(row['性別'])
      row['職業'] = this.getTranslatedWording(row['職業'])
     // row['推定感染経路'] = this.getTranslatedWording(row['推定感染経路'])
